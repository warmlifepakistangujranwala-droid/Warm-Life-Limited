...
export default function BlogsPage(){return <><Navbar/><main className="innerPage"><PageHero eyebrow="Warm Life journal" title="Clear guidance for a more efficient home." description="Practical articles to help households understand energy performance and make informed improvement decisions."/><section className="contentSection"><div className="shell blogGrid">{posts.map(([tag,title,text],i)=><article className="blogCard" key={title}><div className="blogNumber">{String(i+1).padStart(2,"0")}</div><small>{tag}</small><h2>{title}</h2><p>{text}</p><span>Guide coming soon</span></article>)}</div></section></main><Footer/></>}
