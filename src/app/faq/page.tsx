...
export default function FaqPage(){return <><main className="innerPage"><PageHero eyebrow="FAQs" title="Straight answers before you get started." description="Common questions about suitability, eligibility and the home improvement process."/><section className="contentSection"><div className="shell faqList">{faqs.map(([q,a],i)=><article key={q}><span>{String(i+1).padStart(2,"0")}</span><div><h2>{q}</h2><p>{a}</p></div></article>)}</div></section></main></>}
